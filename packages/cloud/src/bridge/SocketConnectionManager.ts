...
import { ConnectionState, type RetryConfig } from '../types.js';
...
export class SocketConnectionManager {
  ...
  public async connect(): Promise<void> {
    ...
    if (
      ...
      console.log(
        `[SocketConnectionManager] Connection attempt already in progress`,
      );
  ...
  private async startConnectionAttempt() {
    ...
    try {
      ...
      console.error(
        `[SocketConnectionManager] Initial connection attempts failed:`,
        error,
      );
  ...
  private async connectWithRetry(): Promise<void> {
    ...
    while (this.retryAttempt < this.retryConfig.maxInitialAttempts) {
      try {
        ...
        console.log(
          `[SocketConnectionManager] Connected to ${this.options.url}`,
        );
        ...
        console.error(
          `[SocketConnectionManager] Connection attempt ${this.retryAttempt} failed:`,
          error,
        );
        ...
        if (this.retryAttempt >= this.retryConfig.maxInitialAttempts) {
          ...
          throw new Error(
            `Failed to connect after ${this.retryConfig.maxInitialAttempts} attempts`,
          );
        }

        console.log(
          `[SocketConnectionManager] Waiting ${delay}ms before retry...`,
        );
  ...
  private async connectSocket(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.socket.on('disconnect', (reason: string) => {
        console.log(
          `[SocketConnectionManager] Disconnected (reason: ${reason})`,
        );
        ...
        if (!isManualDisconnect && this.hasConnectedOnce) {
          ...
          // reconnection.
          console.log(
            `[SocketConnectionManager] Socket.IO will handle reconnection (reason: ${reason})`,
          );
        }
      });
      ...
      this.socket.on('reconnect', (attemptNumber: number) => {
        console.log(
          `[SocketConnectionManager] Socket reconnected (attempt: ${attemptNumber})`,
        );
      ...
      this.socket.on('reconnect_error', (error: Error) => {
        console.error(
          `[SocketConnectionManager] Socket.IO reconnect error:`,
          error,
        );
      });
      ...
      this.socket.on('reconnect_failed', () => {
        console.error(
          `[SocketConnectionManager] Socket.IO reconnection failed after all attempts`,
        );
