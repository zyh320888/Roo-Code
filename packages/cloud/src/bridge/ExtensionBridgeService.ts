...
import type { TaskProviderLike, TaskLike } from '@roo-code/types';
...
export class ExtensionBridgeService {
    ...
    if (userInfo?.extensionBridgeEnabled && remoteControlEnabled) {
      ...
      if (!existingService) {
        try {
          ...
          logger?.(
            `[ExtensionBridgeService#handleRemoteControlState] Instance created (state: ${state})`,
          );
      ...
      if (existingService) {
        try {
          ...
          logger?.(
            `[ExtensionBridgeService#handleRemoteControlState] Service disconnected and reset`,
          );
        } catch (error) {
  ...
  private setupSocketListeners() {
    ...
    socket.on(
      ExtensionSocketEvents.RELAYED_COMMAND,
      (message: ExtensionBridgeCommand) => {
        console.log(
    ...
    socket.on(
      TaskSocketEvents.RELAYED_COMMAND,
      (message: TaskBridgeCommand) => {
        console.log(
  ...
  private async handleConnect() {
    ...
    if (!socket) {
      console.error(
        '[ExtensionBridgeService] Socket not available after connect',
      );
  ...
  private async handleReconnect() {
    ...
    if (!socket) {
      console.error(
        '[ExtensionBridgeService] Socket not available after reconnect',
      );
  ...
  public async subscribeToTask(task: TaskLike): Promise<void> {
    ...
    if (!socket || !this.connectionManager.isConnected()) {
      console.warn(
        '[ExtensionBridgeService] Cannot subscribe to task: not connected. Will retry when connected.',
      );
