...
export class TaskManager {
  ...
  private taskListeners: Map<
    string,
    Map<TaskBridgeEventName, TaskEventListener>
  > = new Map();
  ...
  public async onConnect(socket: Socket): Promise<void> {
    ...
    for (const taskId of subscribedTaskIds) {
      try {
        ...
        console.log(
          `[TaskManager] emit() -> ${TaskSocketEvents.JOIN} ${taskId}`,
        );
      } catch (error) {
        console.error(
  ...
  public async unsubscribeFromTask(
    taskId: string,
    socket: Socket,
  ): Promise<void> {
    ...
    try {
      ...
      console.log(
        `[TaskManager] emit() -> ${TaskSocketEvents.LEAVE} ${taskId}`,
      );
    } catch (error) {
      console.error(
  ...
  public handleTaskCommand(message: TaskBridgeCommand): void {
    ...
    if (!task) {
      console.error(
        `[TaskManager#handleTaskCommand] Unable to find task ${message.taskId}`,
      );
  ...
  private setupListeners(task: TaskLike): void {
    if (this.taskListeners.has(task.taskId)) {
      console.warn(
        '[TaskManager] Listeners already exist for task, removing old listeners:',
        task.taskId,
      );
    ...
    console.log(
      '[TaskManager] Task listeners setup complete for:',
      task.taskId,
    );
  }
  ...
  private removeListeners(task: TaskLike): void {
    ...
    listeners.forEach((listener, eventName) => {
      try {
        ...
        console.error(
          `[TaskManager] Error removing listener for ${String(eventName)} on task ${
            task.taskId
          }:`,
          error,
        );
