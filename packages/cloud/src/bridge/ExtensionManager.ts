...
export class ExtensionManager {
  ...
  public handleExtensionCommand(message: ExtensionBridgeCommand): void {
    if (message.instanceId !== this.instanceId) {
      console.log(
        `[ExtensionManager] command -> instance id mismatch | ${this.instanceId}`,
        {
    ...
    switch (message.type) {
      case ExtensionBridgeCommandName.StartTask: {
        console.log(
          `[ExtensionManager] command -> createTask() | ${message.instanceId}`,
          {
      ...
      case ExtensionBridgeCommandName.StopTask: {
        ...
        if (instance.task.taskStatus === TaskStatus.Running) {
          console.log(
            `[ExtensionManager] command -> cancelTask() | ${message.instanceId}`,
          );
          ...
          console.log(
            `[ExtensionManager] command -> clearTask() | ${message.instanceId}`,
          );
      ...
      case ExtensionBridgeCommandName.ResumeTask: {
        console.log(
          `[ExtensionManager] command -> resumeTask() | ${message.instanceId}`,
          {
  ...
  private setupListeners(): void {
    ...
    eventMapping.forEach(({ from, to }) =>
      this.provider.on(from, addListener(to)),
    );
  }
  ...
  private async publishEvent(message: ExtensionBridgeEvent): Promise<boolean> {
    ...
    try {
      ...
      console.log(
        `[ExtensionManager] emit() -> ${ExtensionSocketEvents.EVENT} ${message.type}`,
        message,
      );
  ...
  private updateInstance(): ExtensionInstance {
    ...
    this.extensionInstance = {
      ...this.extensionInstance,
      appProperties:
        this.extensionInstance.appProperties ?? this.provider.appProperties,
      gitProperties:
        this.extensionInstance.gitProperties ?? this.provider.gitProperties,
      lastHeartbeat: Date.now(),
      task: task
        ? {
            taskId: task.taskId,
